...
from codemcp.tools.code_entity import read_class, read_function, write_class, write_function
...
async def test_read_function_standalone(temp_git_repo):
    ...
    test_file = Path(temp_git_repo) / "test_file.py"
    function_code = await read_function(str(test_file), "standalone_function", "test-chat-id")
...
async def test_read_function_method(temp_git_repo):
    ...
    test_file = Path(temp_git_repo) / "test_file.py"
    function_code = await read_function(str(test_file), "another_method", "test-chat-id", "TestClass")
...
async def test_write_class_new(temp_git_repo):
    ...
    result = await write_class(str(test_file), "NewClass", new_class_code, "Add new class", "test-chat-id")
...
async def test_write_class_existing(temp_git_repo):
    ...
    await write_class(str(test_file), "AnotherClass", new_class_code, "Add another class", "test-chat-id")
    ...
    result = await write_class(str(test_file), "AnotherClass", updated_class_code, "Update class", "test-chat-id")
...
async def test_write_function_new(temp_git_repo):
    ...
    result = await write_function(str(test_file), "new_function", new_function_code, "Add new function", "test-chat-id")
...
async def test_write_function_class_not_exists(temp_git_repo):
    ...
    # Verify the result message
    assert "Successfully created method 'my_method' in class 'NewlyCreatedClass'" in result
    
    # Verify the file was created and contains the expected content
    assert os.path.exists(test_file)
    with open(test_file, "r") as f:
        content = f.read()
        assert "class NewlyCreatedClass:" in content
