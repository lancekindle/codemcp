...
async def parse_file(
    file_path: str,
) -> Tuple[str, ast.AST, Dict[str, Any], Dict[str, Any]]:
...
async def read_class(file_path: str, class_name: str, chat_id: str) -> str:
    ...
    class_source = get_entity_source(
        content, class_data["lineno"], class_data["end_lineno"]
    )
...
async def read_function(
    ...
    if class_name:
        ...
        if function_name not in class_info[class_name]["methods"]:
            raise ValueError(
                f"Method '{function_name}' not found in class '{class_name}' in {full_file_path}"
            )

        # Get the method definition lines
        method_data = class_info[class_name]["methods"][function_name]
        function_source = get_entity_source(
            content, method_data["lineno"], method_data["end_lineno"]
        )
    else:
        # Check if the function exists
        if function_name not in function_info:
            raise ValueError(
                f"Function '{function_name}' not found in {full_file_path}"
            )

        # Get the function definition lines
        function_data = function_info[function_name]
        function_source = get_entity_source(
            content, function_data["lineno"], function_data["end_lineno"]
        )
...
async def write_class(
    ...
    if not creating_new_file:
        try:
            ...
            if class_name in class_info:
                ...
                end_line = class_data[
                    "end_lineno"
                ]  # end_lineno is already the next line
    ...
    return (
        f"Successfully {action} class '{class_name}' in {full_file_path}{git_message}"
    )
        ...
        try:
            ...
            if class_name:
                ...
                if class_name in class_info:
                    ...
                    # For a new file with a new class, just use the straightforward approach
                    # by creating the whole file content directly
                    class_header = f"class {class_name}:"

                    # Then indent the method properly
                    method_lines = new_code.splitlines()
                    # Remove any leading whitespace from the first line of the method
                    method_first_line = method_lines[0].lstrip()
                    indented_method_lines = ["    " + method_first_line]

                    # Indent the rest of the method content
                    if len(method_lines) > 1:
                        for line in method_lines[1:]:
                            indented_method_lines.append("    " + line)

                    # For a new file, we'll create it with proper class and method structure
                    if creating_new_file:
                        # Combine class header with indented method
                        class_code = (
                            class_header + "\n" + "\n".join(indented_method_lines)
                        )
                        # Write the file directly
                        await write_text_content(
                            full_file_path, class_code, "utf-8", line_endings
                        )
                        ...
                        # Return success message for class creation
                        return f"Successfully created method '{function_name}' in class '{class_name}' in {full_file_path}{git_message}"

                    # For existing file with class creation, continue with the normal approach
                    # Combine class header with indented method
